defmodule CircleCI.Plugin do
  ...
  **stack**. To modify the behavior of the client, applications can add or modify plugins.
  ...
  Plugins are functions that accept a `CircleCI.Operation` struct — and any options — and return
  either an `:ok` or an `:error` tuple:

      alias CircleCI.{Error, Operation}
  ...
  the operation's options (order matters!), then calling `CircleCI.Config.plugin_config/4` to
  incorporate the application environment as a fallback. (For required options, the sibling
  function `CircleCI.Config.plugin_config!/3` is also available.)

  In this way, plugins can be configured at all levels of the code:

      # Highest precedence: options from the caller
      CircleCI.Repos.get("aj-foster", "open-api-github", some: :option)
